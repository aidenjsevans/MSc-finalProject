...
void main() {
  ...
  testWidgets('Successfully add bouldering route to project library', (WidgetTester tester) async {
    ...
    await binding.watchPerformance(() async {
      ...
      final listTileCenter = tester.getCenter(boulderingRouteListTile);
      final listTileOffset = listTileCenter + Offset(75,0);

      await tester.tapAt(listTileOffset);
      ...
      final reviewTextButton = find.byKey(Key('reviewTextButton'));
      await tester.scrollUntilVisible(reviewTextButton, 800);
      expect(reviewTextButton, findsOneWidget);

      await Future.delayed(Duration(seconds: 1));

      await tester.tap(reviewTextButton);

      await tester.pumpAndSettle();

      final reviewTextField = find.byKey(Key('reviewTextField'));
      expect(reviewTextField, findsOneWidget);

      final reviewSlider = find.byKey(Key('reviewSlider'));
      expect(reviewSlider, findsOneWidget);

      final reviewRatingBar = find.byKey(Key('reviewRatingBar'));
      expect(reviewRatingBar, findsOneWidget);

      final starIcon = find.byKey(Key('starIcon4'));
      expect(starIcon, findsOneWidget);

      final submitReviewButton = find.byKey(Key('submitReviewButton'));
      expect(starIcon, findsOneWidget);

      String reviewText = 'This climb was good';

      await tester.enterText(reviewTextField, reviewText);
      ...
      final reviewSnackBar = find.byKey(Key('successSnackBar'));
      expect(reviewSnackBar, findsOneWidget);
    }, reportKey: 'review_bouldering_route_performance');
  });
}